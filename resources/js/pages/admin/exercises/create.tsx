...
interface Props {
    // Riceve la lista dal DB per popolare il menu a tendina
...
export default function CreateExercise({ muscleGroups }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // PUNTA ALLA ROTTA CORRETTA DEGLI ESERCIZI
        post('/admin/exercises', { onSuccess: () => reset() });
    };
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-extrabold tracking-tighter text-foreground flex items-center gap-3">
                                <Dumbbell className="text-primary shrink-0" size={28} /> Nuovo Esercizio
                            </h1>
...
=======
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 rounded-2xl border border-sidebar-border bg-sidebar shadow-sm">
                        ...
                        <div className="space-y-3 md:col-span-1">
                            <label className="text-[10px] font-black tracking-[0.2em] text-primary block ml-1"> Nome Esercizio </label>
                            <input 
                                type="text" 
                                value={data.name} 
                        ...
                        <div className="space-y-3 md:col-span-1">
                            ...
                            <div className="relative">
                                <select 
                                    ...
                                    className={`w-full rounded-xl border bg-background p-4 pr-10 focus:border-primary focus:ring-1 focus:ring-primary font-black text-xs transition-all outline-none italic appearance-none cursor-pointer ${errors.muscle_group_id ? 'border-red-500' : 'border-sidebar-border'}`}
                                    required
                                >
                        ...
                        <div className="space-y-3 md:col-span-2">
                            <label className="text-[10px] font-black tracking-[0.2em] text-muted-foreground block ml-1"> Descrizione (Opzionale) </label>
                            <textarea 
                                rows={4} 
                                value={data.description} 
                                onChange={e => setData('description', e.target.value)} 
                                className={`w-full rounded-xl border bg-background p-4 focus:border-primary focus:ring-1 focus:ring-primary transition-all font-bold text-sm outline-none italic resize-none ${errors.description ? 'border-red-500' : 'border-sidebar-border'}`} 
                                placeholder="INSERISCI UNA BREVE SPIEGAZIONE TECNICA..." 
                            />
