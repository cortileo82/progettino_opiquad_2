...
export default function CreatePlan({ client, exercises_list }: Props) {
    const { data, setData, post, processing } = useForm({
        ...
        name: '',
        num_weeks: 4,
        exercises: [{ exercise_id: '', day_of_week: 'Lunedì', sets: '', reps: '', rest_time: '' }]
    });

    const addRow = () => setData('exercises', [...data.exercises, { exercise_id: '', day_of_week: 'Lunedì', sets: '', reps: '', rest_time: '' }]);
            ...
            <div className="p-4 md:p-8 max-w-5xl mx-auto">
                ...
                <form onSubmit={(e) => { e.preventDefault(); post('/pt/plans/store'); }} className="space-y-6">
                    ...
                    <div className="bg-sidebar p-6 rounded-2xl border border-sidebar-border shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Durata (Settimane)</label>
                            <input type="number" value={data.num_weeks} onChange={e => setData('num_weeks', parseInt(e.target.value))} className="w-full bg-background border-sidebar-border rounded-xl px-4 py-2" />
                        </div>
                    </div>

                    {/* Lista Esercizi Dinamica */}
                    <div className="space-y-3">
                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-orange-500 pl-1">Esercizi</h3>
                        {data.exercises.map((row, i) => (
                            <div key={i} className="bg-sidebar p-5 rounded-2xl border border-sidebar-border grid grid-cols-1 md:grid-cols-12 gap-4 items-end shadow-sm hover:border-orange-500/30 transition-colors">
                                ...
                                <div className="md:col-span-5 grid grid-cols-3 gap-2">
                                    <input type="number" value={row.sets} onChange={e => updateRow(i, 'sets', e.target.value)} placeholder="Serie" className="w-full bg-background border-sidebar-border rounded-lg text-sm p-2 text-center border-none" />
                                    <input type="text" value={row.reps} onChange={e => updateRow(i, 'reps', e.target.value)} placeholder="Reps" className="w-full bg-background border-sidebar-border rounded-lg text-sm p-2 text-center border-none" />
                                    <input type="text" value={row.rest_time} onChange={e => updateRow(i, 'rest_time', e.target.value)} placeholder="Rec." className="w-full bg-background border-sidebar-border rounded-lg text-sm p-2 text-center border-none" />
                                </div>
