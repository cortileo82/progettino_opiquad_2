            ...
            <div className="flex h-full flex-col gap-8 p-6 md:p-10">
                ...
                <div className="grid gap-4 mt-4">
                    {availableClients.length > 0 ? (
                        availableClients.map((client) => (
                            <div key={client.id} className="flex items-center justify-between rounded-[2rem] border border-sidebar-border bg-sidebar p-6 shadow-sm hover:border-primary/20 transition-colors">
                                <div className="flex flex-col gap-1">
                                    <span className="font-black uppercase italic text-lg tracking-tight text-foreground"> {client.name}</span>
                                ...
                                <button 
                                    ...
                                    className="inline-flex items-center justify-center gap-3 rounded-2xl bg-primary px-6 py-3 text-[10px] font-black uppercase italic tracking-widest text-primary-foreground hover:opacity-90 shadow-xl transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed border border-sidebar-border"
                                >
                        ...
                        <EmptyState message={search ? `Nessun risultato per "${search}"` : "Nessun nuovo atleta disponibile"} icon={Users} />
                    )}
                </div>
            </div>
        </AppLayout>
